...
export type HookType = 'clean' | 'extract' | 'navigate' | 'stop';

const CLEAN_PROMPT = `你是一个网页清理专家。下面提供了页面的 Accessibility Tree（JSON 格式），包含页面中所有可见元素的角色（role）、名称（name）、id、class、href 等属性信息。
请根据这棵树生成一个在浏览器中运行的 JavaScript IIFE，用于从页面中移除不相关的元素（如导航栏、侧边栏、广告、页脚、弹窗等），只保留正文内容。
注意：此脚本会直接修改页面 DOM。
返回格式必须是 { removed: number }，表示移除了多少个元素。
只返回代码，不要任何解释。`;
...
function getPromptTemplate(hookType: HookType): string {
  switch (hookType) {
    case 'clean': return CLEAN_PROMPT;
...
function getHookLabel(hookType: HookType): string {
  switch (hookType) {
    case 'clean': return '清理脚本 (Clean Hook)';
