...
const EXTRACT_PROMPT = `你是一个网页内容提取专家。根据以下页面 DOM 结构，
生成一个在浏览器中运行的 JavaScript IIFE，用于提取页面内容。
重要：不要修改原始页面 DOM。如果需要移除元素，先用 cloneNode(true) 克隆后再操作。
返回格式必须是 { title: string, html: string } 或 null（提取失败时）。
只返回代码，不要任何解释。`;

const NAVIGATE_PROMPT = `你是一个网页自动化专家。根据以下页面 DOM 结构，
生成一个在浏览器中运行的 JavaScript IIFE，用于执行页面导航操作。
返回格式必须是 { success: boolean }。
success=true 表示导航操作已执行，false 表示找不到导航目标（如按钮不存在、被禁用）。
只返回代码，不要任何解释。`;

const STOP_PROMPT = `你是一个网页自动化专家。根据以下页面 DOM 结构，
生成一个在浏览器中运行的 JavaScript IIFE，用于判断批量采集是否应该终止。
