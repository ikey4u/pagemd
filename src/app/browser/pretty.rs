...
- **`browser_eval` defaults to `record_undo: false`** — keep it false for read-only probes (`document.querySelector`, checks). Set `true` only when mutating DOM (in-page mutation recording for `/undo`).
