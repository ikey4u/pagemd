...
| `browser_eval` | Run JS; **`record_undo: false`** for read-only checks (fast); `true` when mutating DOM |
...
impl CursorRelay {
    pub fn send_user_line(&self, line: &str) -> Result<()> {
        self.0.run_turn(line)
    }

    pub fn send_context_block(&self, block: &str) -> Result<()> {
        self.send_user_line(block)
    }
